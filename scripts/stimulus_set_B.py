...
# Cylinder
cylinder = [
    CrossSection(controlpoints=cs_2, position=position) for position in np.linspace(0.0, 0.95, NUM_CROSS_SECTION)
]

# Point
scale = np.ones((NUM_CROSS_SECTION, 1))
scale[-5:] *= np.linspace(1, 0.1, 5).reshape(-1, 1)
position = np.linspace(0.0, 0.98, NUM_CROSS_SECTION)
point = [CrossSection(controlpoints=scale[i] * cs_2, position=position[i]) for i in range(NUM_CROSS_SECTION)]

# Football
scale = np.geomspace(1, 3, NUM_CROSS_SECTION // 2)
scale = np.concatenate([scale, scale[-1::-1]])
position = np.linspace(0.0, 0.95, NUM_CROSS_SECTION)
football = [CrossSection(controlpoints=scale[i] * cs_2, position=position[i]) for i in range(NUM_CROSS_SECTION)]

# Hourglass
scale = np.geomspace(1, 3, NUM_CROSS_SECTION // 2)
scale = np.concatenate([scale[-1::-1], scale])
position = np.linspace(0.0, 0.95, NUM_CROSS_SECTION)
hourglass = [CrossSection(controlpoints=scale[i] * cs_2, position=position[i]) for i in range(NUM_CROSS_SECTION)]
...
for b_k, b_v in two_segment_backbone_dict.items():
    for cs_k, cs_v in cs_dict.items():
        projection_key = b_k + "_" + cs_k
        projection_value = AxialComponent(b_v, cs_v, smooth_with_post=False)
        two_segment_projection_dict[projection_key] = projection_value

##############
### Shapes ###
##############

pairs = [list(tup) for tup in itertools.combinations_with_replacement(two_segment_projection_dict.keys(), 2)]
...
shapes = []
for i, (p1_key, p2_key, T_rot, B_rot) in enumerate(combs[:10]):

    print(i, p1_key, p2_key, T_rot, B_rot)

    p1 = copy.deepcopy(two_segment_projection_dict[p1_key])
    p2 = copy.deepcopy(two_segment_projection_dict[p2_key])
